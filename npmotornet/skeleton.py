...
class Skeleton:
  ...
  def __init__(self, dof: int, space_dim: int, name: str = "skeleton",
    ...
    self.default_endpoint_load = np.zeros((1, self.space_dim), dtype=np.float32)
  ...
  def build(
    ...
    :class:`npmotornet.effector.Effector` object class or subclass.
    ...
    self.pos_upper_bound = np.array(pos_upper_bound, dtype=np.float32).reshape(1, -1)
    self.pos_lower_bound = np.array(pos_lower_bound, dtype=np.float32).reshape(1, -1)
    self.vel_upper_bound = np.array(vel_upper_bound, dtype=np.float32).reshape(1, -1)
    self.vel_lower_bound = np.array(vel_lower_bound, dtype=np.float32).reshape(1, -1)
    self.dt = timestep
  ...
  def path2cartesian(self, path_coordinates, path_fixation_body, joint_state):
    ...
    This method is used by the wrapper :class:`npmotornet.effector.Effector` object class or subclass to then
    ...
    Args:
      ...
      path_fixation_body: The body part (bone) to which each muscle fixation point is attached to. This is an
        attribute held by the wrapper :class:`npmotornet.effector.Effector` object class or subclass.
  ...
  def integrate(self, dt: float, state_derivative, joint_state):
    ...
    :meth:`npmotornet.effector.Effector.integration_step` during numerical integration by the
    :class:`npmotornet.effector.Effector` wrapper class or subclass.
    ...
    Args:
      ...
      state_derivative: `Array`, derivative of joint state. This input and `joint_state` should have identical
        dimensionality.
      joint_state: `Array`, joint state that serves as the initial value. This input and `state_derivative`
  ...
  def joint2cartesian(self, joint_state):
    ...
    Returns:
      An `array` containing the current cartesian configuration (position, velocity).
    """
  ...
  def ode(self, inputs, joint_state, endpoint_load):
    ...
    Args:
      inputs: `Array`, the control input to the skeleton object (typically torques).
      joint_state: `Array`, the current joint configuration.
      endpoint_load: `Array`, the loads applied to the skeleton's endpoint. The dimensionality should match the
  ...
  def clip_velocity(self, pos, vel):
    ...
    Args:
      pos: An `array` containing positions of the plant in joint space.
      vel: An `array` containing velocities of the plant in joint space.

    Returns:
      An `array` containing the clipped velocities, with same dimensionality as the `vel` input argument.
    """
    vel = self.clip(vel, self.vel_lower_bound, self.vel_upper_bound)
    vel = np.where(np.logical_and(vel < 0, pos <= self.pos_lower_bound), 0., vel)
    vel = np.where(np.logical_and(vel > 0, pos >= self.pos_upper_bound), 0., vel)
    return vel
...
class PointMass(Skeleton):
  ...
  def __init__(self, space_dim: int = 2, mass: float = 1., name: str = "point_mass", **kwargs):
    ...
    dpos_ddof = np.eye(self.dof, dtype=np.float32)[np.arange(0, self.dof) % self.dof][np.newaxis, :, :, np.newaxis]
  ...
  def _path2cartesian(self, path_coordinates, path_fixation_body, joint_state):
    pos, vel = np.split(joint_state[:, :, np.newaxis], 2, axis=1)
    # if fixed on the point mass, then add the point-mass position / velocity to the fixation point coordinate
    pos = np.where(path_fixation_body == 0, 0., pos) + path_coordinates
    vel = np.where(path_fixation_body == 0, 0., vel)
    dpos_ddof = np.where(path_fixation_body == 0, 0., self.dpos_ddof)
...
class FixedPointMass(Skeleton):
  """A fixed point-mass skeleton for isometric testing.

  This skeleton maintains a fixed position regardless of muscle forces, making it ideal
  for studying muscle force production under isometric (constant length) conditions.
  Unlike the regular PointMass which accelerates when muscles contract, this skeleton
  remains stationary, allowing tendon forces to build up fully.

  This is particularly useful for:
  - Validating force-firing relationships in sensory receptors (e.g., GTOs)
  - Studying maximum voluntary contraction
  - Testing muscle models under controlled conditions

  Args:
    space_dim: `Integer`, the dimensionality of the space in which the point-mass exists.
      For instance, this would be `2` for a point-mass in a cartesian, planar `xy` space.
    fixed_position: `Array` or `list`, the fixed position of the point-mass. If None,
      defaults to origin (all zeros). Shape should match space_dim.
    name: `String`, the name of the skeleton.
    **kwargs: This is passed as-is to the parent :class:`Skeleton` class.
  """

  def __init__(self, space_dim: int = 2, fixed_position=None, name: str = "fixed_point_mass", **kwargs):
    super().__init__(dof=space_dim, space_dim=space_dim, name=name, **kwargs)

    # Fixed position (defaults to origin)
    if fixed_position is None:
      self.fixed_position = np.zeros((1, space_dim), dtype=np.float32)
    else:
      self.fixed_position = np.array(fixed_position, dtype=np.float32).reshape(1, -1)

    # Pre-compute values for path2cartesian method (same as PointMass)
    dpos_ddof = np.eye(self.dof, dtype=np.float32)[np.arange(0, self.dof) % self.dof][np.newaxis, :, :, np.newaxis]
    self.dpos_ddof = dpos_ddof
  ...
  def _integrate(self, dt, state_derivative, joint_state):
    ...
    fixed_pos = np.tile(self.fixed_position, (batch_size, 1))
    fixed_vel = np.zeros((batch_size, self.dof), dtype=np.float32)
    return np.concatenate([fixed_pos, fixed_vel], axis=1)

  def _path2cartesian(self, path_coordinates, path_fixation_body, joint_state):
    # Same as PointMass since the logic is identical
    pos, vel = np.split(joint_state[:, :, np.newaxis], 2, axis=1)
    # if fixed on the point mass, then add the point-mass position / velocity to the fixation point coordinate
    pos = np.where(path_fixation_body == 0, 0., pos) + path_coordinates
    vel = np.where(path_fixation_body == 0, 0., vel)
  ...
  def get_save_config(self):
    """Gets the base configuration from the :meth:`Skeleton.get_base_config` method,
    and adds the fixed position.

    Returns:
      A `dictionary` containing the object instance's full configuration.
    """
    cfg = self.get_base_config()
    cfg['fixed_position'] = self.fixed_position.tolist()
    return cfg
...
class TwoDofArm(Skeleton):
  ...
  def __init__(self, name: str = 'two_dof_arm', m1: float = 1.864572, m2: float = 1.534315, l1g: float = 0.180496,
    ...
    self.inertia_c = inertia_c.reshape((1, 2, 2))
  ...
  def _ode(self, inputs, joint_state, endpoint_load):
    ...
    coriolis = np.stack([coriolis_1, coriolis_2], axis=1)
  ...
  def _path2cartesian(self, path_coordinates, path_fixation_body, joint_state):
    ...
    dx_da1 = np.where(path_fixation_body == 0., 0., dx_da) + np.where(path_fixation_body == 2., -elb_y, 0.)
    dy_da1 = np.where(path_fixation_body == 0., 0., dy_da) + np.where(path_fixation_body == 2., elb_x, 0.)
    dx_da2 = np.where(path_fixation_body == 2., dx_da, 0.)
    dy_da2 = np.where(path_fixation_body == 2., dy_da, 0.)

    dxy_da1 = np.concatenate([dx_da1, dy_da1], axis=1)
    dxy_da2 = np.concatenate([dx_da2, dy_da2], axis=1)
    dxy_da = np.concatenate([dxy_da1[:, :, None, :], dxy_da2[:, :, None, :]], axis=2)
    ...
    bone_origin = np.where(path_fixation_body == 2, np.concatenate([elb_x, elb_y], axis=1), 0.)
    xy = np.concatenate([dy_da, -dx_da], axis=1) + bone_origin
